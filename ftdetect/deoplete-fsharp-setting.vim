augroup deoplete-fsharp
    ...
    autocmd  InsertLeave        *.fs,*.fsi,*.fsx  :silent execute ":write! " . temporary_file_path
    autocmd  InsertEnter        *.fs,*.fsi,*.fsx  :silent execute ":write! " . temporary_file_path 
    autocmd  BufWrite           *.fs,*.fsi,*.fsx  :silent execute ":write! " . temporary_file_path
augroup END

function! s:create_temporary_file() abort
    let temporary_file_path = substitute( expand('%:p:r') . '_deoplete-fsharp_temporary_file.fsx' , '\#', '\\#' , 'g' )
    execute ":silent write! " . temporary_file_path 
endfunction
